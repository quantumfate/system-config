...
| projects.json                | declaration | which projects exist and what each opens: path, kitty window template, workspace, declared scopes — hand-curated (`,proj.sh add` includes a directory deliberately, `drop` removes one; `sync` only refreshes what each stored project's own `.proj.toml` declares, it discovers nothing; `,proj.sh` reads it at runtime), plus dashboard metadata (kind, study, priority)                                                                                                                                                                            |
